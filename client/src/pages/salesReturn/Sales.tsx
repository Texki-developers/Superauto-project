...
import { IGetSales } from '../../types/vehicle/sellVehicle';
import EditSellPage from './EditSales';
...
const SalesReturn = () => {
  ...
  const [selectedVehicleData, setSelectedVehicleData] = useState<IGetSales | null>(null)
  const onAddButtonClick = () => {
    setShowAddPage(true);
  };
  const [searchParams] = useSearchParams()
  const { callApi } = useGetApis()
  const url = `inventory/sales?page=${searchParams.get('page') ?? 1}&perPage=${searchParams.get('perPage') ?? 10}`
  ...
  const onActionClick = (type: 'add' | 'edit' | 'delete', id: string, data: IGetSales) => {
    if (type === 'add') {
    ...
    <>
      ...
      {showDeletePage &&
        ...
        <DeleteModal
          ...
          label={`Enter the Vehicle Registration Number ( ${selectedVehicleData?.Inventory?.registration_number} )`}
        />
      }
